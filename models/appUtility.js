...
const a = async () => {
  ...
  //   { $match: { busNumber } }, // Match documents with the specified busNumber
  //   { $unwind: "$previousFiveContributor" }, // Split the array into separate documents
  //   { $sort: { "previousFiveContributor.createdAt": -1 } }, // Sort by createdAt in descending order
  //   {
  //     $group: {
  //       _id: "$_id", // Group by the _id field of the parent document
  //       currentContributor: { $first: "$currentContributor" }, // Keep the currentContributor
  //       previousFiveContributor: { $push: "$previousFiveContributor" }, // Reassemble the sorted array
  //     },
  //   },
  // ]);
};
